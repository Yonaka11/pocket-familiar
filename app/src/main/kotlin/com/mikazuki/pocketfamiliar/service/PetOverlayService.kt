...
class PetOverlayService : Service() {
    ...
    private val displayListener = object : DisplayManager.DisplayListener {
        override fun onDisplayChanged(displayId: Int) {
            if (isOverlayRunning) updateScreenDimensions()
        }
    ...
    private fun startOverlay() {
        ...
        overlayManager.setProfile(PetRegistry.getById(settings.selectedPetId))

        physics.apply {
            screenWidth  = sw;  screenHeight  = sh;  bottomInsetPx = nb
    ...
    private fun onPetStateChanged(state: PetState) {
        ...
        // When entering Jumping, give the pet a horizontal impulse away from the wall.
        if (state is PetState.Jumping) {
            val wasOnLeft = physics.x <= 1f
            physics.launchFromEdge(fromLeftWall = wasOnLeft)
        }
    }

    private fun onDragStarted(startX: Float, startY: Float) {
        physics.x = startX; physics.y = startY
        stateMachine.forceState(PetState.Dragged)
    }

    private fun onDragMoved(x: Float, y: Float) {
        physics.x = x; physics.y = y
    }
