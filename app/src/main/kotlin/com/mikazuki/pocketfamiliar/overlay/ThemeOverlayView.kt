...
class ThemeOverlayView(context: Context) : View(context) {
    ...
    private fun drawSakura(canvas: Canvas) {
        repeat(24) { i ->
            ...
            val x = xBase + sin(elapsedSeconds * 0.9f + i) * 34f
            val size = 5f + (i % 4) * 1.7f
            softPaint.color = if (i % 3 == 0) Color.rgb(255, 205, 220) else Color.rgb(255, 170, 198)
            softPaint.alpha = 185
            drawPetal(canvas, x, y, size, elapsedSeconds * 40f + i * 19f)
        }
    }
    ...
    private fun drawTechFrame(canvas: Canvas) {
        ...
        val inset = 14f * d
        val corner = 58f * d
        val pulse = (170 + 70 * (0.5f + 0.5f * sin(elapsedSeconds * 2.3f))).toInt()
        ...
        fun cornerPath(left: Float, top: Float, sx: Float, sy: Float) {
            ...
            canvas.drawLine(left + sx * corner * 0.36f, top + sy * 8f * d, left + sx * corner * 0.72f, top + sy * 8f * d, linePaint)
        }
        ...
        linePaint.alpha = 180
        repeat(6) { i ->
            val y = height * (0.18f + i * 0.13f)
            canvas.drawLine(inset, y, inset + (8f + i % 3 * 6f) * d, y, linePaint)
            canvas.drawLine(width - inset, y, width - inset - (8f + i % 3 * 6f) * d, y, linePaint)
        }
    }
    ...
    private fun drawCozyGlow(canvas: Canvas) {
        ...
        repeat(18) { i ->
            ...
            softPaint.alpha = 90 + (i % 4) * 24
            canvas.drawCircle(x, y, (2.5f + i % 3) * d, softPaint)
        }
    }
    ...
    private fun drawCelestial(canvas: Canvas) {
        ...
        repeat(3) { ring ->
            ...
            repeat(4 + ring * 2) { i ->
                ...
                softPaint.alpha = 175
                canvas.drawCircle(x, y, 1.8f * d, softPaint)
            }
            canvas.restore()
        }
        drawEdgeStars(canvas, Color.rgb(255, 231, 155), 16)
    }
    ...
    private fun drawMoonDust(canvas: Canvas) {
        ...
        repeat(32) { i ->
            ...
            softPaint.alpha = 65 + (i % 5) * 24
            canvas.drawCircle(x, y, (1.1f + i % 3 * 0.7f) * d, softPaint)
        }
    }
    ...
    private fun drawBloomGarden(canvas: Canvas) {
        ...
        repeat(18) { i ->
            ...
            softPaint.alpha = 140
            canvas.drawCircle(x, if (i % 3 == 0) topY else bottomY, (2.3f + i % 3) * d, softPaint)
        }
        ...
        drawEdgeStars(canvas, Color.rgb(255, 218, 126), 10)
    }
    ...
    private fun drawEdgeStars(canvas: Canvas, color: Int, count: Int) {
        ...
        repeat(count) { i ->
            ...
            softPaint.alpha = (190 * pulse).toInt()
            canvas.drawCircle(x, y, (1.5f + i % 3) * d, softPaint)
        }
    }
